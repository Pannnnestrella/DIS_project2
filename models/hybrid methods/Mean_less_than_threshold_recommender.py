...
def evaluate_parameters(user_item_matrix_train, user_similarity_df, item_similarity_df, 
                        test_data_part, user_rating_counts, book_rating_counts, params):
    """
    Evaluate the model with given parameters, dynamically calculating global mean.
    """
    alpha, threshold_book, threshold_user = params
    
    # Identify low-activity users and books based on current thresholds
    low_activity_users = [user for user, count in user_rating_counts.items() if count < threshold_user]
    low_activity_books = [book for book, count in book_rating_counts.items() if count < threshold_book]
    
    # Filter ratings for low-activity users and books
    low_activity_ratings = user_item_matrix_train.loc[low_activity_users, low_activity_books]
    
    # Dynamically calculate global mean for low-activity ratings
    if threshold_user == 1:
        # Ignore NaN for users; use item means
        global_mean_threshold = user_item_matrix_train.mean(axis=0).mean()
    elif threshold_book == 1:
        # Ignore NaN for books; use user means
        global_mean_threshold = user_item_matrix_train.mean(axis=1).mean()
    else:
        # Standard mean calculation
        global_mean_threshold = low_activity_ratings.stack().mean()
    ...
    for _, row in test_data_part.iterrows():
        predicted_rating = predict_combined_with_threshold(
            ...
            user_rating_counts, book_rating_counts, global_mean_threshold,
    ...
    return rmse, global_mean_threshold
...
def grid_search_parameters(train_data):
    ...
    threshold_books = [2, 3, 4, 5,7,10]  # Based on book rating quartiles
    threshold_users = [2, 3, 4, 5,7,10]  # Based on user rating quartiles
    
    best_rmse = float('inf')
    best_params = None
    
    # Create all combinations of parameters (without precomputing global mean)
    ...
    for params in tqdm(param_combinations, desc="Evaluating parameter combinations"):
        rmse, global_mean_threshold = evaluate_parameters(
            user_item_matrix_train, user_similarity_df, item_similarity_df,
        ...
        if rmse < best_rmse:
            ...
            print(f"Global Mean: {global_mean_threshold:.4f}, RMSE: {rmse:.4f}")
...
if __name__ == "__main__":
    ...
    print(f"Global Mean: {best_params[3]}")
